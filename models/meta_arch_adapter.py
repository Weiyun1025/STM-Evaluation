...
from .adapter_modules import SpatialPriorModule, Injector, Extractor
...
class MetaArchAdapter(nn.Module):
    r""" ConvNeXt
    ...
    def __init__(self,
        ...
        self.vit_stem = SpatialPriorModule(dims=dims, vit_dim=vit_dim)
        ...
        for i, (depth, dim) in enumerate(zip(depths, dims)):
            ...
            self.injectors.append(Injector(dim=dim, vit_dim=vit_dim))
            self.stages.append(stage)
            self.extractors.append(Extractor(dim=dim, vit_dim=vit_dim))
    ...
    def forward_features(self, x):
        ...
        # shape: (B, C, H, W)
        vit_tokens, _, _ = self.vit_stem(x)
        for i in range(4):
            x = self.downsample_layers[i](x)
            x = self.injectors[i](x, vit_tokens)
