...
class HaloAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if halo == 0:
            kv_inp = rearrange(x, 'b c (h p1) (w p2) -> (b h w) (p1 p2) c', p1=block, p2=block)
        else:
...
class HaloBlockV3(nn.Module):
    def __init__(self, dim, drop_path, layer_scale_init_value, depth,
                 block_size, halo_size, stage, num_heads,
                 mlp_ratio=4., drop=0., head_dim=None, act_layer=nn.GELU, norm_layer=LayerNorm2d,
                 **kwargs):
        super().__init__()
        self.dim = dim
