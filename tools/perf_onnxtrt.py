...
def main(args):
    ...
    ts_model = '{}/{}.ts'.format(root, args.model_name)
    ...
    print('** platform_has_tf32:', builder.platform_has_tf32)
    print('** platform_has_fast_fp16:', builder.platform_has_fast_fp16)
    print('** platform_has_fast_int8:', builder.platform_has_fast_int8)
    print('** num_DLA_cores:', builder.num_DLA_cores)
    print('** max_DLA_batch_size:', builder.max_DLA_batch_size)
    ...
    # build trt engines
    fp32_trt_model = build_trt_engine(builder,
                                      network,
                                      args.input_shape,
                                      tf32=False,
                                      fp16=False)
    tf32_trt_model = build_trt_engine(builder,
                                      network,
                                      args.input_shape,
                                      tf32=True,
                                      fp16=False)
    fp16_trt_model = build_trt_engine(builder,
                                      network,
                                      args.input_shape,
                                      tf32=False,
                                      fp16=True)
    ...
    throught, max_diff = perf_trt_engine(fp32_trt_model, logger, input_data,
                                         output_gt)
    print('onnx trt fp32: \tqps: {}, \tdiff: {}'.format(throught, max_diff))
    throught, max_diff = perf_trt_engine(tf32_trt_model, logger, input_data,
                                         output_gt)
    print('onnx trt tf32: \tqps: {}, \tdiff: {}'.format(throught, max_diff))
    throught, max_diff = perf_trt_engine(fp16_trt_model, logger, input_data,
                                         output_gt)
    print('onnx trt fp16: \tqps: {}, \tdiff: {}'.format(throught, max_diff))
    throught, max_diff = perf_trt_engine(tf32_fp16_trt_model, logger,
                                         input_data, output_gt)
    print('onnx trt tf32-fp16: \tqps: {}, \tdiff: {}'.format(
        throught, max_diff))
